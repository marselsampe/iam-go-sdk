...
func Test_DefaultClientValidateAndParseClaims_PlatformAppID(t *testing.T) {
	...
	assert.Equal(t, "1234567", claims.PlatformAppID, "paid should be parsed into PlatformAppID")
	...
	require.NoError(t, err, "access token is invalid")
	require.NotNil(t, claims, "claims should not nil")
	assert.Empty(t, claims.PlatformAppID, "PlatformAppID should be empty when the token carries no paid claim")
}
